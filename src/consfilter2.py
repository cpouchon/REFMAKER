...
parser.add_argument("--Ht",help="maximal shared heterozygous site (accross samples)",
                    type=float)
parser.add_argument("--infos",help="minimal informative sites frequency",
                    type=float)
...
parser.add_argument("-R", help="minimum percentage of populations required to process a locus",
                    type=float)
parser.add_argument("-t", help="minimal taxa per locus",
                    type=int)
...
bads_contigs_info_sites=list()
suminfo="summary.txt"
open(os.path.join(outpath, suminfo), 'w').close()
with open(os.path.join(outpath,suminfo), 'a+') as file:
    file.write("Name"+"\t"+"Seqs"+"\t"+"Sites"+"\t"+"Infor"+"\t"+"Invar"+"\t"+"Amb"+"\t"+"Miss"+"\t"+"MissAll"+"\n")

print('Selecting loci according to informative sites proportion')
Bar = ProgressBar(len(ref_list), 60, '\t ')
barp=0
    ...
    if contig in bads:
        ...
        if contig in list(clean_catalog_1.keys()):
            ...
            for b in range(bmin,bmax):
                ...
                for s in clean_catalog_1[contig]:
                    totlen+=1
                    tmpsite.append(str(clean_catalog_1[contig][s])[b])
                    if str(clean_catalog_1[contig][s])[b]=="-":
                        pass
                    else:
                        if str(clean_catalog_1[contig][s])[b].upper() in ht_basis:
                            pass
                        else:
                            tmpsite_without_miss.append(str(clean_catalog_1[contig][s])[b])

                amb=sum([tmpsite.count(k) for k in ht_basis])
                totamb+=amb
                totlen_all+=len(sampling)
                miss=tmpsite.count("-")
                totmiss+=miss
                totmiss_all+=miss+(len(sampling)-len(clean_catalog_1[contig]))
            ...
            fmiss_tot=round((totmiss_all/totlen_all*100),2)
            fmiss_sel=round((totmiss/totlen)*100,2)
            pinfos=float(totinfo)/float(window)
            if pinfos>=minfos and len(clean_catalog_1[contig])>=mtaxa:
                partnumber=partnumber+1
                start=end+1
                end=(start-1)+ref_len[contig]
                with open(os.path.join(outpath,suminfo), 'a+') as file:
                    file.write(str("part"+str(partnumber)+"\t"+str(len(clean_catalog_1[contig]))+"\t"+str(window)+"\t"+str(totinfo)+"\t"+str(totinvar)+"\t"+str(famb)+"\t"+str(fmiss_sel)+"\t"+str(fmiss_tot)+"\n"))
                    file.close()
                ...
                bads_contigs_info_sites.append(contig)
    Bar.update(barp)
print("")
print(str("%s/%s loci removed according to the informative sites proportion" % (len(bads_contigs_info_sites),len(ref_list)-len(bads))+"\n"))
...
if condsamp==0 and condcont==0:
    ...
    Bar = ProgressBar(len(sampling), 60, '\t ')
    barp=0
    with open(os.path.join(outpath,logname), 'a+') as file:
    ...
    for samp in sampling:
        ...
        concatenate_seq=""
        for contig in ref_list:
            if contig in bads or contig in bads_contigs_info_sites:
                continue
            else:
                if contig in list(clean_catalog_1.keys()):
    ...
    print("loci number: %s" % (len(ref_list)-len(bads)-len(bads_contigs_info_sites)))
    print("samples: %s" % (fsamp))
    print("length (bp): %s" % (flen))
    with open(os.path.join(outpath,logname), 'a+') as file:
        file.write(str("[INFOS]: final matrix")+"\n")
        file.write(str("loci number: %s" % (len(ref_list)-len(bads)-len(bads_contigs_info_sites)))+"\n")
