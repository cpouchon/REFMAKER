...
print("max. heterozygosity frequency per locus: %s" % (str(seq_h)))
with open(os.path.join(outpath,logname), 'a+') as file:
    file.write(str("max. heterozygosity frequency per locus: %s" % (str(seq_h)))+"\n")
    ...
    if lenseq<mlen:
        ...
        for samp in sampling:
            if samp in list(seqs.keys()):
                ...
                cond1=sequence.count("-")/lenseq
                cond2=sum([sequence.count(k) for k in ht_basis])/lenseq
                if cond_outgrp>0:
...
## ici changer car les chiffres ne correspondent pas dans good_outgp_shared --> ds good_outgp_shared il y a des mauvais
